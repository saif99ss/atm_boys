class Account < ActiveRecord::Base
 ...
 def deposit(i)
   ...
   if i[:amount].to_f <= 0.0
     ...
     if ((Transaction.where("account_id =? and created_at>=? and 1=?",id,t,true).sum(:amount).to_f)+i[:amount].to_f) > 1000
 ...
 def withdrawal(i)
   ...
   if i[:amount].to_f <= 0.0 
     ...
     if (self.balance.to_f  - i[:amount].to_f < 0.0 )
       ...
       if ((Transaction.where("account_id =? and created_at>=? and 2=?",id,t,false).sum(:amount).to_f)+i[:amount].to_f) > 500
